...
class FTraceCallGraph:
	...
	def addLine(self, line, match):
		...
		if len(self.list) >= 1000000 or self.depth < 0:
			...
			if (self.depth < 0):
				print(f'Too much data for {id} (buffer overflow), ignoring this callback')
			else:
				print(f'Too much data for {id} {window}, ignoring this callback')
			return False
		self.list.append(line)
		if(self.start < 0):
			self.start = line.time
...
def initFtraceAndroid():
	...
	if sysvals.usetraceevents:
		...
		tp = sysvals.tpath
		# turn trace off
		os.system(f"{sysvals.adb} shell 'echo 0 > {tp}tracing_on'")
		# set the trace clock to global
		os.system(f"{sysvals.adb} shell 'echo global > {tp}trace_clock'")
		# set trace buffer to a huge value
		os.system(f"{sysvals.adb} shell 'echo nop > {tp}current_tracer'")
		os.system(f"{sysvals.adb} shell 'echo 10000 > {tp}buffer_size_kb'")
		# turn trace events on
		events = iter(sysvals.traceevents)
		for e in events:
			os.system(f"{sysvals.adb} shell 'echo 1 > {sysvals.epath}{e}/enable'")
		# clear the trace buffer
